...
export function RecurringTaskModal({
  ...
  const [formValues, setFormValues] = useState<RecurrenceFormValues>(
    value ?? defaultValues,
  );
  ...
  const updateField = <K extends keyof RecurrenceFormValues>(
    key: K,
    newValue: RecurrenceFormValues[K],
  ) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t(language, "repeatTitle")}>
      <div className="space-y-4">
        <div className="space-y-2">
          <label className="text-sm font-medium">
            {t(language, "repeatMode")}
          </label>
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) =>
                updateField(
                  "frequency",
                  e.target.value as RecurrenceFormValues["frequency"],
                )
              }
            >
        ...
        {formValues.mode === "scheduled" && (
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              {t(language, "repeatStartDateHint")}
            </p>
          </div>
        )}
        ...
        <div className="flex justify-between gap-2 pt-2">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={handleSave}>{t(language, "repeatSave")}</Button>
          </div>
        </div>
      </div>
    </Modal>
