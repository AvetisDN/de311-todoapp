...
export const useTodos = create<TodoState>((set, get) => {
  ...
  const generateScheduledTasks = async (currentTodos: Todo[]) => {
    try {
      ...
      for (const recurring of recurringTasks) {
        ...
        const template = currentTodos.find(
          (todo) => todo.id === recurring.todo_id,
        );
        if (!template) continue;

        let nextRunAt = recurring.next_run_at
        ...
        if (nextRunAt <= now) {
          ...
          const nextFuture = advanceToFuture(
            addInterval(nextRunAt, recurring.frequency, recurring.interval),
            recurring.frequency,
            recurring.interval,
          );
          await supabase
            .from("recurring_tasks")
            .update({
              next_run_at: nextFuture.toISOString(),
  ...
  const handleAfterCompletion = async (todo: Todo, completedAt: Date) => {
    ...
    const nextRunAt = getNextScheduledDate(
      baseDate,
      recurring.frequency,
      recurring.interval,
      completedAt,
    ...
    addTodo: async (newTodo) => {
      try {
        ...
        if (recurrence?.reminderTime && todoData.due_date) {
          todoData.reminder_time = applyReminderTime(
        ...
        if (recurrence) {
          ...
          const startDateValue = baseDate
            ...
            : null;
          const nextRunAt =
            recurrence.mode === "scheduled" && baseDate
              ? addInterval(
                  new Date(baseDate),
          ...
          createdTodo = {
            ...createdTodo,
            recurring_task_id: recurringData.id,
          } as Todo;
        }
